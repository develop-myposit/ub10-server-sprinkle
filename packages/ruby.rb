...
package :default_rubies do
  ...
  runner "ruby-build 1.9.3-p392 /opt/ruby-1.9.3-p392"
#  runner "ruby-build 1.8.7-p352 /opt/ruby-1.8.7-p352"
  runner "chown -R #{DEPLOY_USER} /opt/ruby*"
  
  verify do
    @commands << "/opt/ruby-1.9.3-p392/bin/ruby -v"
...
package :default_ruby do
  ...
  runner "ln -s /usr/local/rubies/ruby-1.9.3-p392/bin/* /usr/local/bin/"
  runner "source /etc/profile"

  verify do
    @commands << "ruby -v"
    @commands << "gem -v"
    has_symlink '/usr/local/bin/ruby', '/usr/local/rubies/ruby-1.9.3-p392/bin/ruby' 
    has_executable "ruby"
  end
end
...
package :add_bundler do
  ...
  runner "ln -s /usr/local/rubies/ruby-1.9.3-p392/lib/ruby/gems/1.9.1/gems/bundler-1.3.5/bin/bundle /usr/local/bin/bundle"
  
  verify do 
    @commands << 'gem list | grep bundler' 
    has_symlink '/usr/local/bin/bundle', '/usr/local/rubies/ruby-1.9.3-p392/lib/ruby/gems/1.9.1/gems/bundler-1.3.5/bin/bundle'
  end
end
